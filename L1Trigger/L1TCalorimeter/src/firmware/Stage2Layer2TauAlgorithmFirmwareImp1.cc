...
{  
	...
	if(secClusters.size()>0){
	  ...
	  for (unsigned int iTT = 0; iTT < TTPos.size(); iTT++) {
	    ...
	    if (thisTTItr != TTPos.end()){
	      ...
	      if (secondaryCluster->checkClusterFlag (TTPosRemap.at(iTT)) ) mainClusterTrim.setClusterFlag (TTPosRemap.at(idx), false);
	    }
	...
	tau.setHwPt(seedEt);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NW)) tau.setHwPt(tau.hwPt() + towerEtNW);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_N))  tau.setHwPt(tau.hwPt() + towerEtN);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NE)) tau.setHwPt(tau.hwPt() + towerEtNE);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_E))  tau.setHwPt(tau.hwPt() + towerEtE);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SE)) tau.setHwPt(tau.hwPt() + towerEtSE);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_S))  tau.setHwPt(tau.hwPt() + towerEtS);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SW)) tau.setHwPt(tau.hwPt() + towerEtSW);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_W))  tau.setHwPt(tau.hwPt() + towerEtW);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NN)) tau.setHwPt(tau.hwPt() + towerEtNN);
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SS)) tau.setHwPt(tau.hwPt() + towerEtSS);
	...
	int EtUp   = 0;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NE)) EtUp += towerEtNE;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_N))  EtUp += towerEtN;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NW)) EtUp += towerEtNW;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_NN)) EtUp += towerEtNN;
	int EtDown = 0;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SE)) EtDown += towerEtSE;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_S))  EtDown += towerEtS;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SW)) EtDown += towerEtSW;
	if(mainClusterTrim.checkClusterFlag(CaloCluster::INCLUDE_SS)) EtDown += towerEtSS;
	//
...
void l1t::Stage2Layer2TauAlgorithmFirmwareImp1::dosorting (std::vector<l1t::Tau>& taus)
{
    ...
    for (unsigned int iTau = 0; iTau < taus.size(); iTau++)
    {
      ...
      if (taus.at(iTau).hwEta() > 0) tauEtaPos.at( taus.at(iTau).hwEta()-1).at((72-taus.at(iTau).hwPhi())/4) = taus.at(iTau);
      else                           tauEtaNeg.at( -(taus.at(iTau).hwEta()+1)).at((72-taus.at(iTau).hwPhi())/4) = taus.at(iTau);
